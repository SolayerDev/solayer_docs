...
After an immensely successful Episode 1 that saw over 100,000 restakers join the Solayer ecosystem, we’re back with Episode 2.
...
4. Referred someone who has restaked
5. Restaked an exchange-backed LST (BNSOL, bbSOL and \[redacted])
...
1. **Milestone 1: sSOL goes liquid** - sSOL is now a liquid token that allows for users to transfer and withdraw on Solayer. This unlocks a ton of utility for restakers who can now participate in DeFi with sSOL.
2. **Milestone 2: Solayer Mainnet Deployment** - We launched our endogenous AVS (endoAVS) system on the Solana mainnet as the first restaking platform live on mainnet. This enabled users to delegate their sSOL to endoAVSs and participate in restaking on Solana.
...
4. **Milestone 4: Exchange-backed LSTs launch with restaking on Solayer** - Several exchanges have launched their LSTs and chosen Solayer as the key restaking partner in the launch. Solayer supported restaking for Binance Staked SOL (BNSOL) and Bybit Staked SOL (bbSOL) right out of the gate.
...
Episode 2 is where restakers claim their rewards while they interact with restaking on Solayer. This includes:

1. Percentile Rank - shows where you stand based on your deposit amount relative to other restakers, such as being in the top 10%.
2. Numerical Rank - numerical rank indicates your exact position among all unique deposit addresses.
3. Extra Credits - extra credits provide boosts based on specific actions, such as delegating to an endoAVS, restaking an exchange-backed LST or using sSOL in DeFi.
